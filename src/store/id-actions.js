...
export const  idsave = () => {
    const email=localStorage.getItem('emailtoken').replace('.','').replace('@','').replace('g','');
      ...
      return async (dispatch) => {
        ...
        const sent = async () => {
         const response= await axios.put(
              `https://react-hariom-default-rtdb.firebaseio.com/${email}.json`,add);
         ...
         return data;
            ...
            try {
              ...
              if(data===null){
                ...
                (data.email===email && data.id===id && data.token===token && data.time===time ){
